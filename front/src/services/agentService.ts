...
import type { StreamController } from '@/api/frontend/agent'
...
class AgentService {
  private sessionStore: ReturnType<typeof useAgentSessionStore> | null = null
  private executionStore: ReturnType<typeof useAgentExecutionStore> | null = null
  private uiStore: ReturnType<typeof useAgentUIStore> | null = null
    ...
    try {
      this.streamController = streamAgentChat(
        ...
        },
        {
          ...
          onError: (error: AgentEvent) => this.handleError(error, aiMessageId),
          onComplete: () => this.handleComplete(aiMessageId),
          onReconnect: (info: { retryCount: number }) => this.handleReconnect(info)
        }
      )
    } catch (err: any) {
      this.handleError(
        { payload: { message: err.message, recoverable: false } } as AgentEvent,
        aiMessageId
      )
    }
  }
  ...
  handleEvent(event: AgentEvent, messageId: string): void {
    // 更新执行状态
  ...
  handleError(error: AgentEvent, messageId: string): void {
  ...
  handleComplete(messageId: string): void {
    // 更新消息状态
    this.sessionStore!.updateMessage(messageId, {
      isStreaming: false
    })
  ...
  handleReconnect(info: { retryCount: number }): void {
  ...
  applyQuickCommand(command: QuickCommand): string {
