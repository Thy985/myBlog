<template>
    <div class="tag-list" :style="{ left: $store.state.menuWidth }">
        <el-tabs v-model="activeTab" type="card" style="min-width: 100px;" @tab-remove="removeTab" @tab-change="tabChange">
            <el-tab-pane
...
// 添加标签导航
function addTab(tab) {
    const noTab = tabList.value.findIndex(i => i.path === tab.path) === -1
    if (noTab) {
