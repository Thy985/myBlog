<template>
    <div :class="['flex gap-3 agent-msg-enter', message.role === 'user' ? 'justify-end' : 'justify-start']">
        <div v-if="message.role !== 'user'" class="w-8 h-8 rounded-lg bg-gradient-to-br from-[var(--agent-primary)] to-[#8B5CF6] flex items-center justify-center shadow-glow flex-shrink-0">
        ...
        <div v-if="message.role === 'user'" class="w-8 h-8 rounded-lg bg-gradient-to-br from-[var(--agent-success)] to-[#14B8A6] flex items-center justify-center shadow-glow-success flex-shrink-0">
            <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
.markdown-content :deep(blockquote) { border-left: 3px solid var(--agent-primary); padding-left: 12px; margin: 8px 0; color: var(--agent-text-muted); }
